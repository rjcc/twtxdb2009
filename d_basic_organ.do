...
insheet using "/Users/apple/Documents/rjcc2/txdb2009/d_basic_organ.csv", clear names
...
label values dm htn heart_dz liver_dz kidney_dz drug_abuse head_injury fever shock cpr yn
...
label define bdie 1 "Hypoxia/Asystole" 2 "Head Injury" 3 "CVA" 4 "CNS tumor" 5 "Others" 999 "?"
...
quietly log using d_basic_organ, smcl replace
...
bysort organ_code: tab sex_type y_donor, col
gen age=(donor_date-birthday)/365.24
bysort organ_code: tabstat age, by(y_donor) stat(n mean sd med min max) format(%9.2g)
bysort organ_code: tab blood_type y_donor, col
* Hepatitis=Y if anti_hcv=Y or anti_hbs=Y or hbs_ag=Y or hbe_ag=Y
* Hepatitis=N if all above N
bysort organ_code: tab hepatitis y_donor, col

quietly log close
quietly log2html d_basic_organ, replace erase linesize(255)
